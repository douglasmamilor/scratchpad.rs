...
fn generate_pixels(buf: &mut [u32], rng: &mut Rng) {
    for pixel in buf.iter_mut() {
        *pixel = if rng.next_u64() & 1 == 0 {
            0xFFFFFFFF
        } else {
            0xFF000000
        };
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Clear to aqua, so we know what the background color is
    window.clear(0, 255, 255);
    'running: loop {
        ...
        generate_pixels(frame_buffer.pixels_mut(), &mut rng);
