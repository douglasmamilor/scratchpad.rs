...
fn draw_tiles(renderer: &mut Renderer) {
    for y in (0..renderer.height()).step_by(32) {
        for x in (0..renderer.width()).step_by(32) {
            let is_even = ((x / 32) + (y / 32)) % 2 == 0;
            let color = if is_even { Color::WHITE } else { Color::BLACK };
